...
* CHA only results
reghdfe ntl cha_cleared [aw=pct_area], absorb(absorb_temp) cluster(district_id year)
outreg2 using "$results/main_models_cha.doc", replace noni nocons ctitle(NTL) addtext("Grid cell FEs", N, "Year FEs", N)

reghdfe ntl cha_cleared [aw=pct_area], absorb(year) cluster(district_id year)
outreg2 using "$results/main_models_cha.doc", append noni nocons ctitle(NTL) addtext("Grid cell FEs", N, "Year FEs", Y)

reghdfe ntl cha_cleared [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_cha.doc", append noni nocons ctitle(NTL) addtext("Grid cell FEs", Y, "Year FEs", Y)

reghdfe ntl c.cha_cleared##c.cha_road_blockage  [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_cha.doc", append noni nocons ctitle(NTL) addtext("Grid cell FEs", Y, "Year FEs", Y)


reghdfe popcount cha_cleared [aw=pct_area], absorb(absorb_temp) cluster(district_id year)
outreg2 using "$results/main_models_cha.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", N, "Year FEs", N)

reghdfe popcount cha_cleared [aw=pct_area], absorb(year) cluster(district_id year)
outreg2 using "$results/main_models_cha.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", N, "Year FEs", Y)

reghdfe popcount cha_cleared [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_cha.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", Y, "Year FEs", Y)

reghdfe popcount c.cha_cleared##c.cha_road_blockage  [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_cha.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", Y, "Year FEs", Y)
...
* SHA only results
reghdfe ntl sha_cleared [aw=pct_area], absorb(absorb_temp) cluster(district_id year)
outreg2 using "$results/main_models_sha.doc", replace noni nocons ctitle(NTL) addtext("Grid cell FEs", N, "Year FEs", N)

reghdfe ntl sha_cleared [aw=pct_area], absorb(year) cluster(district_id year)
outreg2 using "$results/main_models_sha.doc", append noni nocons ctitle(NTL) addtext("Grid cell FEs", N, "Year FEs", Y)

reghdfe ntl sha_cleared [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_sha.doc", append noni nocons ctitle(NTL) addtext("Grid cell FEs", Y, "Year FEs", Y)

reghdfe ntl c.sha_cleared##c.sha_road_blockage  [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_sha.doc", append noni nocons ctitle(NTL) addtext("Grid cell FEs", Y, "Year FEs", Y)


reghdfe popcount sha_cleared [aw=pct_area], absorb(absorb_temp) cluster(district_id year)
outreg2 using "$results/main_models_sha.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", N, "Year FEs", N)

reghdfe popcount sha_cleared [aw=pct_area], absorb(year) cluster(district_id year)
outreg2 using "$results/main_models_sha.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", N, "Year FEs", Y)

reghdfe popcount sha_cleared [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_sha.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", Y, "Year FEs", Y)

reghdfe popcount c.sha_cleared##c.sha_road_blockage [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_sha.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", Y, "Year FEs", Y)
...
* results considering both SHA and CHA
reghdfe ntl all_cleared [aw=pct_area], absorb(absorb_temp) cluster(district_id year)
outreg2 using "$results/main_models_all.doc", replace noni nocons ctitle(NTL) addtext("Grid cell FEs", N, "Year FEs", N)

reghdfe ntl all_cleared [aw=pct_area], absorb(year) cluster(district_id year)
outreg2 using "$results/main_models_all.doc", append noni nocons ctitle(NTL) addtext("Grid cell FEs", N, "Year FEs", Y)

reghdfe ntl all_cleared [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_all.doc", append noni nocons ctitle(NTL) addtext("Grid cell FEs", Y, "Year FEs", Y)

reghdfe ntl c.all_cleared##c.any_road_blockage [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_all.doc", append noni nocons ctitle(NTL) addtext("Grid cell FEs", Y, "Year FEs", Y)

reghdfe ntl c.all_cleared##c.baseline_pop [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_all.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", Y, "Year FEs", Y)


reghdfe popcount all_cleared [aw=pct_area], absorb(absorb_temp) cluster(district_id year)
outreg2 using "$results/main_models_all.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", N, "Year FEs", N)

reghdfe popcount all_cleared [aw=pct_area], absorb(year) cluster(district_id year)
outreg2 using "$results/main_models_all.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", N, "Year FEs", Y)

reghdfe popcount all_cleared [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_all.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", Y, "Year FEs", Y)

reghdfe popcount c.all_cleared##c.any_road_blockage [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_all.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", Y, "Year FEs", Y)

reghdfe popcount c.all_cleared##c.baseline_pop [aw=pct_area], absorb(cell_id year) cluster(district_id year)
outreg2 using "$results/main_models_all.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", Y, "Year FEs", Y)
...
reghdfe ntl ibn.q_baseline_pop#c.all_cleared, absorb(cell_id year) cluster(district_id year)
...
reghdfe ntl ibn.q_pct_area#c.all_cleared, absorb(cell_id year) cluster(district_id year)
...
reghdfe ntl i.test, absorb(cell_id year) cluster(district_id year)
estimate store e1
...
reghdfe ntl cha_cleared [aw=pct_area], absorb(year) cluster(province_id year)
...
reghdfe ntl c.cha_cleared##c.cha_road_blockage [aw=pct_area], absorb(district_id year) cluster(province_id year)
outreg2 using "$results/main_models_cha_commune.doc", append noni nocons ctitle(NTL) addtext("Grid cell FEs", Y, "Year FEs", Y)
...
reghdfe popcount cha_cleared [aw=pct_area], absorb(year) cluster(province_id year)
...
reghdfe popcount c.cha_cleared##c.cha_road_blockage [aw=pct_area], absorb(district_id year) cluster(province_id year)
outreg2 using "$results/main_models_cha_commune.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", Y, "Year FEs", Y)
...
reghdfe ntl sha_cleared [aw=pct_area], absorb(year) cluster(province_id year)
...
reghdfe ntl c.sha_cleared##c.sha_road_blockage [aw=pct_area], absorb(district_id year) cluster(province_id year)
outreg2 using "$results/main_models_sha_district.doc", append noni nocons ctitle(NTL) addtext("Grid cell FEs", Y, "Year FEs", Y)
...
reghdfe popcount sha_cleared [aw=pct_area], absorb(year) cluster(province_id year)
...
reghdfe popcount c.sha_cleared##c.sha_road_blockage [aw=pct_area], absorb(district_id year) cluster(province_id year)
outreg2 using "$results/main_models_sha_district.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", Y, "Year FEs", Y)
...
reghdfe ntl all_cleared [aw=pct_area], absorb(year) cluster(province_id year)
...
reghdfe ntl c.all_cleared##c.any_road_blockage [aw=pct_area], absorb(district_id year) cluster(province_id year)
outreg2 using "$results/main_models_all_district.doc", append noni nocons ctitle(NTL) addtext("Grid cell FEs", Y, "Year FEs", Y)
...
reghdfe popcount all_cleared [aw=pct_area], absorb(year) cluster(province_id year)
...
reghdfe popcount c.all_cleared##c.any_road_blockage [aw=pct_area], absorb(district_id year) cluster(province_id year)
outreg2 using "$results/main_models_all_district.doc", append noni nocons ctitle(Pop. count) addtext("Grid cell FEs", Y, "Year FEs", Y)
